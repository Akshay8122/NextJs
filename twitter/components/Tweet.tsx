...
function Tweet({ tweet }: Props) {
  ...
  return (
    <div className="flex flex-col space-x-3 border-y p-5 border-gray-100 ">
        ...
        <div>
          ...
          {tweet.image && (
            <img
              src={tweet.image}
              alt=""
              className="m-5 ml-10 mb-1 max-h-60 rounded-lg object-cover shadow-sm"
            />
          )}
        </div>
      </div>
      ...
      {commentBoxVisible && (
        <form className="mt-3 flex space-x-3" onSubmit={handleComment}>
          <input
            ...
            type="text"
            placeholder="Write a commenrt ..."
          />
          <button
            disabled={!input}
            type="submit"
            className="text-twitter disabled::text-gary-200"
          >
            Post
          </button>
        </form>
      ...
      {comments?.length > 0 && (
        <div className="my-2 mt-5 max-h-44 space-y-5 overflow-y-scroll-hidden border-t border-garay-100 p-5 ">
